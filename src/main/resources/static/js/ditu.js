...
(function(e) {
	e.fn.smartZoom = function(t) {
		...
		function u() {
			...
			if (parseFloat(r * 100) > parseFloat(e.adjustedPosInfos.scale * 100))
				n.css({
					cursor : "move"
				});
			else
		...
		function l(e) {
			...
			if (t.mouseMoveForPan || !t.mouseMoveForPan
					&& t.moveCurrentPosition.x != e.pageX
					&& t.moveCurrentPosition.y != e.pageY) {
		...
		function c(t) {
			...
			if (r.mouseMoveForPan) {
				...
				if (r.moveLastPosition.distance(r.moveCurrentPosition) > 4) {
					var i = r.moveLastPosition.interpolate(
							r.moveCurrentPosition, -4);
					v(i.x, i.y, 500, true)
				} else {
		...
		function d(t) {
			...
			if (i.touch.touchMove) {
				...
				if (i.settings.dblTapEnabled == true
						...
						&& (new Date).getTime() - i.touch.lastTouchEndTime < 400) {
		...
		function g(t) {
			...
			if (r.transitionObject) {
				if (r.transitionObject.cssAnimHandler)
					n
							.off(e.support.transition,
									r.transitionObject.cssAnimTimer);
				var i = r.originalSize;
				var s = S();
				var o = new Object;
				o[r.transitionObject.transition] = "all 0s";
				if (r.transitionObject.css3dSupported) {
					setFlag(s.x,s.y,s.width/ i.width,s.height / i.height);
					o[r.transitionObject.transform] = "translate3d(" + s.x
							+ "px, " + s.y + "px, 0) scale3d(" + s.width
							/ i.width + "," + s.height / i.height + ", 1)"
				} else {
					setFlag(s.x,s.y,s.width/ i.width,s.height / i.height);
					o[r.transitionObject.transform] = "translateX(" + s.x
							+ "px) translateY(" + s.y + "px) scale(" + s.width
							/ i.width + "," + s.height / i.height + ")"
				}
				n.css(o)
			} else {
				n.stop()
		...
		function w() {
			...
			t.width(N(e.settings.width, pwidth, s - i.left));
			t.height(N(e.settings.height, pheight, o - i.top));
			var a = L(t);
			...
			g();
			while(l<window.innerWidth||c<window.innerHeight){
				l=l*1.5;
				c=c*1.5;
			}
			var initW=(window.innerWidth-l)/2;
			var initH=(window.innerHeight-c)/2;
			/*console.info(n+"=="+e.adjustedPosInfos.left+"=="+e.adjustedPosInfos.top+"=="+l+"=="+c);*/
			E(n,initW, initH, l, c, 0,
					function() {
		...
		function E(t, r, i, s, o, u, a) {
			...
			c=c>0?0:c;
			h=h>0?0:h;
			var mw=window.innerWidth-s;
			var mh=window.innerHeight-o;
			c=c<mw?mw:c;
			h=h<mh?mh:h;
			if (f.transitionObject != null) {
				...
				d[f.transitionObject.transition] = "all " + u / 1e3
						+ "s ease-out";
				if (f.transitionObject.css3dSupported){
					setFlag(c,h,s / p.width,o / p.height);
					d[f.transitionObject.transform] = "translate3d(" + c
							+ "px, " + h + "px, 0) scale3d(" + s / p.width
							+ "," + o / p.height + ", 1)";}
				else{
					setFlag(c,h,s / p.width,o / p.height);
					d[f.transitionObject.transform] = "translateX(" + c
							+ "px) translateY(" + h + "px) scale(" + s
							/ p.width + "," + o / p.height + ")";}
				if (a != null) {
					f.transitionObject.cssAnimHandler = a;
					t.one(e.support.transition.end,
							f.transitionObject.cssAnimHandler)
				}
				t.css(d)
			} else {
				t.animate({
		...
		function S(e) {
			...
			var i = n.height();
			var s = n.offset();
			var o = parseFloat(s.left);
			var u = parseFloat(s.top);
		...
		var i = {
			init : function(t) {
				...
				if (settings.mouseEnabled == true) {
					...
					if (settings.scrollEnabled == true) {
						...
						u.bind("mousewheel.smartZoom DOMMouseScroll.smartZoom",
								o)
					}
					if (settings.dblClickEnabled == true
							&& settings.zoomOnSimpleClick == false)
			...
			zoom : function(e, t, i) {
				...
				var mins =Math.max(window.innerWidth/2756,window.innerHeight/1969);
				h = Math.max(mins, h);
			...
			destroy : function() {
				...
				i.before(n);
				E(n, t.originalPosition.left, t.originalPosition.top,
...
$.fn.emulateTransitionEnd = function(e) {
	...
	var r = function() {
		if (!t)
			$(n).trigger($.support.transition.end)
	};
	setTimeout(r, e);
	return this
};
